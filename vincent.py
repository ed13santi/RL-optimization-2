...
def local_ders_tf(x, local_ders, N):
    tmpList = []
    for i in range(N):
...
class Actor:
    def __init__(self, alpha_theta, loc_state_size):        
        self.learning_rate = alpha_theta
        self.loc_state_size = loc_state_size
        self.NN = self._init_NN()
        self.target_NN = self._init_targetNN()

    def _init_NN(self):
        model = tf.keras.Sequential()
        model.add(layers.Dense(16, input_shape=(self.loc_state_size,), activation="relu", name="layer1"))
...
class Critic:
    ...
    def _init_NN(self):
        model = tf.keras.Sequential()
        model.add(layers.Dense(16, input_shape=(self.loc_state_size+1,), activation="relu", name="layer1"))
        model.add(layers.Dense(16, activation="relu", name="layer2"))
        model.add(layers.Dense(1, activation="sigmoid", name="layer3"))
...
def state_trans(s,a, C_tf, constr_A_tf, max_x, delta_l, N):
    # features of state
    x_plus_m = s[:N]
    lambdas = s[N:]
...
def rewards_parallel(global_state, xs, constr_A_tf, local_ders, N):
    lambdas = global_state[N:]
    dUidxis = local_ders_tf(xs, local_ders, N)
...
def global2i_states(state, N):
    lambdas = tf.slice(state, [N], [-1])
...
def main():    
    ...
    actors = [Actor(alpha_theta, loc_state_size) for _ in range(N)]
    critics = [Critic(alpha_thetaCritic, loc_state_size) for _ in range(N)]
    ...
    for episode in range(n_episodes):
        ...
        for it in range(episode_length):
            ...
            local_states = global2i_states(state, N)
            ...
            new_state, xs = state_trans(state, noisy_actions, C_tf, constr_A_tf, max_x, delta_l, N)
            r = rewards_parallel(new_state, xs, constr_A, local_ders, N)
            rs.append(r)
            new_local_states = global2i_states(new_state, N)
            #sess = tf.compat.v1.Session()
            for i in range(N):
                new_entry = tf.concat((local_states[i], noisy_actions[i]*tf.ones((1,)), r[i]*tf.ones((1,)), new_local_states[i]), axis=0)
                #new_entry_eval = sess.run(new_entry)
                replay_mem[i] = tf.concat((replay_mem[i], tf.expand_dims(new_entry, axis=0)), axis=0) #CHANGE new_entry to new_entry_eval
            ...
            if (it + 1) % update_every == 0 and episode * episode_length + it + 1 >= batch_size: 
                for i in range(N):
                    ...
                    #choices = tf.constant(random.choices(range(replay_mem_len), k=batch_size), shape=(batch_size,))
                    choices = random.choices(range(replay_mem_len), k=batch_size)
                    batch = tf.gather(params=replay_mem[i], indices=choices, axis=0)
                    state_arr = batch[:,:1+K]
